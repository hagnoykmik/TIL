...
- 인접 형제 결합자(+)
  - selectorA의 형제 요소 중 바로 뒤에 위치하는(아래) 요소
```html
<style>
  p + span {
    color: red;
